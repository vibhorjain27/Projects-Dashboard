...
function buildCard(project, delay = 0) {
  ...
  // Click card body (not buttons) → open modal
  on(card, 'click', e => {
    if (!e.target.closest('.btn')) openModal(project);
  });
...
function updateStats() {
  ...
  const professional = PROJECTS.filter(p => p.category === 'professional').length;
  const personal     = PROJECTS.filter(p => p.category === 'personal').length;

  animateCount($('stat-total'),        total);
  animateCount($('stat-live'),         live,  900);
  animateCount($('stat-professional'), professional, 1000);
  animateCount($('stat-personal'),     personal, 1100);
}
